...
import datasets.ngc_defect as ngc
...
def main():
    ...
    for class_name in ngc.CLASS_NAMES:

        train_dataset = ngc.NGC_Defect_Dataset(args, class_name=class_name, is_train=True)
        train_dataloader = DataLoader(train_dataset, batch_size=args.batch_size, pin_memory=True)
        test_dataset = ngc.NGC_Defect_Dataset(args, class_name=class_name, is_train=False)
        ...
        for (x, y) in tqdm(test_dataloader, f'| feature extraction | test | {class_name} |'):
        ...
        with open('defect_scores_0901/'+args.arch+'_'+class_name+'.pkl','wb') as f:
        ...
        plot_fig(test_imgs, scores, None, None, save_dir, class_name)
...
#    test_img = [test_img]
    ...
    for i in range(num):
        ...
        ax = ax_img[1].imshow(heat_map, cmap='jet', alpha=0.5, interpolation='none', norm=norm)
